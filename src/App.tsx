...
function App() {
  ...
  const writeErrorStrings = {
    my: {
      ...
      duplicateMsg: 'သင့်တွင် တက်ကြွသော တောင်းခံချက် တစ်ခု ရှိပြီးဖြစ်သည်။ ၄င်းကို ပိတ်ပြီးမှ အသစ်တင်နိုင်ပါသည်။',
    ...
    en: {
      ...
      duplicateMsg: 'You already have an active blood request. Close it before posting a new one.',
  ...
  const handlePosted = async (draft: RequestDraft) => {
    ...
    if (error) {
      if (error.code === '23505') {
        ...
        setWriteError({ title: errStrings.duplicateTitle, message: errStrings.duplicateMsg })
      } else {
