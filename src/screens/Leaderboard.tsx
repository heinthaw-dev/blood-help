 ...
 */
export function Leaderboard({ lang, onNavigate, currentUserId }: LeaderboardProps) {
  const isMy = lang === 'my'
  ...
  useEffect(() => {
    ...
    async function loadLeaderboard() {
      const { data, error } = await supabase.rpc('leaderboard_top_donors', { p_limit: ROW_LIMIT })
      if (cancelled) return
      if (error || !data) {
      ...
      setRows(
        data.map((d) => ({
          ...
          rank: d.rank,
          isUser: currentUserId != null && d.profile_id === currentUserId,
        })),
      )
  ...
  const t = {
    my: {
      title: 'လူ့အသက်များ ကယ်တင်ခဲ့သူများ',
      subtitle: 'လှူဒါန်းမှုတိုင်းသည် တစ်စုံတစ်ဦးအတွက် ဒုတိယအခွင့်အရေး ဖြစ်ပါသည်',
  ...
  return (
    <div className="phone-entry-stage">
      <div className="phone-entry-card" style={{ height: '100dvh' }}>
        {/* Header */}
        <div style={{ flex: 'none', padding: '26px 20px 14px', background: 'var(--color-bg)' }}>
        ...
        {/* Scrollable body */}
        <div style={{ flex: 1, minHeight: 0, overflowY: 'auto', scrollbarWidth: 'none', padding: '4px 20px 24px' }}>
          {/* Title block */}
          <div style={{ textAlign: 'center', padding: '6px 4px 0' }}>
            <h1 style={{ margin: 0, fontFamily: bodyFont, fontSize: 23, fontWeight: 600, lineHeight: 1.35, color: 'var(--text-primary)' }}>
              {t.title}
            </h1>
            <p style={{ margin: '12px auto 0', maxWidth: 300, fontFamily: bodyFont, fontSize: 14, lineHeight: 1.6, color: 'var(--text-secondary)' }}>
              {t.subtitle}
            </p>
          </div>

          {loading ? (
            <div style={{ textAlign: 'center', padding: '48px 24px', fontFamily: bodyFont, fontSize: 14, color: 'var(--text-hint)' }}>
              {t.loading}
            </div>
          ) : rows.length === 0 ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', padding: '48px 24px 24px' }}>
              ...
              >
                <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="var(--color-primary)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ display: 'block' }}>
                  <path d="M12 2.7s6 6 6 10.3a6 6 0 0 1-12 0c0-4.3 6-10.3 6-10.3z" />
                </svg>
              </div>
              <div style={{ fontFamily: bodyFont, fontSize: 18, fontWeight: 600, lineHeight: 1.6, color: 'var(--text-primary)', maxWidth: 280 }}>
                {t.lowData}
              </div>
            </div>
          ) : (
            <>
              ...
              >
                ...
                >
                  <svg width="20" height="20" viewBox="0 0 24 24" fill="var(--color-primary)" style={{ display: 'block' }}>
                    <path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z" />
                  </svg>
                </div>
                <div style={{ minWidth: 0, fontFamily: bodyFont, fontSize: 15, lineHeight: 1.5, color: 'var(--text-primary)' }}>
                  {t.impactPrefix}
                  <strong style={{ fontWeight: 600, color: 'var(--color-primary)' }}>{formatNumber(livesSaved, lang)}</strong>
                  {t.impactSuffix}
                </div>
              </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginTop: 22 }}>
                {top3.map((row) => {
                  const medal = MEDALS[MEDAL_BY_RANK[row.rank - 1] ?? 'bronze']
                  ...
                  return (
                    ...
                    >
                      ...
                      <div style={{ position: 'relative', flex: 'none' }}>
                        {isFirst && (
                          <svg width="26" height="18" viewBox="0 0 26 18" fill="none" style={{ position: 'absolute', left: '50%', top: -15, transform: 'translateX(-50%)', display: 'block' }}>
                            <path d="M2 4.5l4.5 4 6.5-7 6.5 7 4.5-4-2 12H4L2 4.5z" fill={medal.ring} stroke={medal.ring} strokeWidth="1.4" strokeLinejoin="round" />
                            <circle cx="2" cy="4.5" r="1.8" fill={medal.ring} />
                            <circle cx="24" cy="4.5" r="1.8" fill={medal.ring} />
                            <circle cx="13" cy="1.6" r="1.8" fill={medal.ring} />
                          </svg>
                        )}
                        <div
                          style={{
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                          ...
                          {row.isUser && <span style={youPill}>{t.you}</span>}
                        </div>
                        <div style={{ ...bloodPill, marginTop: 6 }}>{row.bloodType}</div>
                      </div>

                      {/* Count */}
                      <div style={{ flex: 'none', textAlign: 'right' }}>
                        <div style={{ fontFamily: burmeseFont, fontSize: isFirst ? 28 : 24, fontWeight: 600, lineHeight: 1, color: 'var(--text-primary)' }}>
                          {formatNumber(row.count, lang)}
                        </div>
                        <div style={{ fontFamily: burmeseFont, fontSize: 13, color: 'var(--text-secondary)', marginTop: 3 }}>
                          {t.times}
                        </div>
                      </div>
                    </div>
              ...
              {rest.length > 0 && (
                <>
                  <div style={{ display: 'flex', alignItems: 'baseline', gap: 8, margin: '26px 2px 12px' }}>
                    <span style={{ fontFamily: bodyFont, fontSize: 16, fontWeight: 600, color: 'var(--text-primary)' }}>
                      {t.allDonors}
                    </span>
                  </div>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                    {rest.map((row) => (
                      ...
                      >
                        {/* rank */}
                        <div style={{ width: 24, flex: 'none', textAlign: 'center', fontFamily: 'var(--font-sans)', fontSize: 15, fontWeight: 700, lineHeight: 1, color: 'var(--text-hint)' }}>
                          {row.rank}
                        </div>
                        {/* avatar */}
                        <div
                        ...
                        <div style={{ flex: 1, minWidth: 0, display: 'flex', alignItems: 'center', gap: 8 }}>
                          ...
                          {row.isUser && <span style={youPill}>{t.you}</span>}
                          <span style={{ ...bloodPill, padding: '4px 8px' }}>{row.bloodType}</span>
                        </div>
                        {/* count */}
                        <div style={{ flex: 'none', textAlign: 'right' }}>
                          ...
                          <span style={{ fontFamily: burmeseFont, fontSize: 16, color: 'var(--text-secondary)' }}> {t.times}</span>
                        </div>
                      </div>
                    ))}
                  </div>
